...
# 3b. BASE DE DATOS ALARMAS - PAQUETES BÁSICOS HOGAR
tarifas_alarmas = [
    {"PRIORIDAD": 1, "COMPAÑÍA": "SEGURMA", "PLAN": "Tranquilidad (Básico)", "CUOTA": "43,90€/mes IVA incl. (Promo 19,90€/mes los primeros 12 meses)", "INSTALACION": "0€", "PERMANENCIA": "24 meses", "ELEMENTOS": "Central + 2 PIR cámara + 1 PIR + Teclado + Sirena + 3 TAGs + Mando", "logo": "manuales/logo_segurma.png"},
    {"PRIORIDAD": 2, "COMPAÑÍA": "VERISURE", "PLAN": "Kit Básico Hogar", "CUOTA": "49,80€/mes IVA incl.", "INSTALACION": "Gratis (Renove con alarma activa grado 2)", "PERMANENCIA": "—", "ELEMENTOS": "Panel + Mandos + Detector apertura + Fotodetectores + 1 elemento a elegir gratis", "logo": "manuales/logo_verisure.png"},
    {"PRIORIDAD": 3, "COMPAÑÍA": "ADT SMART SECURITY", "PLAN": "Basic Hogar", "CUOTA": "42,35€/mes IVA incl.", "INSTALACION": "Incluida", "PERMANENCIA": "24 meses", "ELEMENTOS": "Panel HUB + Detector movimiento con cámara + Detector magnético + Teclado + 4 llaves inteligentes", "logo": "manuales/logo_adt.png"},
    {"PRIORIDAD": 4, "COMPAÑÍA": "PROSEGUR (MOVISTAR)", "PLAN": "SuperSeguridad Hogar", "CUOTA": "48,90€/mes IVA incl.", "INSTALACION": "399€ (promocionada)", "PERMANENCIA": "24 meses", "ELEMENTOS": "Panel + Cámara 360º + Detectores + Garantía Antiocupación y Antirrobo incluidas 24 meses", "logo": "manuales/logo_prosegur.png"},
    {"PRIORIDAD": 5, "COMPAÑÍA": "3D SEGURIDAD", "PLAN": "Básico", "CUOTA": "24,20€/mes IVA incl. los primeros 12 meses, después 37,51€/mes IVA incl.", "INSTALACION": "Gratis", "PERMANENCIA": "36 meses", "ELEMENTOS": "Sistema personalizado según necesidades del cliente, +25 años de experiencia, CRA propia", "logo": "manuales/logo_3d.png"},
]
...
with st.sidebar:
    ...
    "Navegación",
    ["🚀 CRM", "📊 PRECIOS", "⚡ COMPARADOR ENERGÍA", "📶 COMPARADOR TELCO", "🛡️ COMPARADOR ALARMAS", "📢 ANUNCIOS Y PLAN AMIGO", "📈 DASHBOARD Y RANKING", "📂 REPOSITORIO", "🕒 CONTROL LABORAL", "🔐 ZONA DIRECTIVOS"]
)
...
# --- COMPARADOR ALARMAS ---
elif menu == "🛡️ COMPARADOR ALARMAS":
    st.markdown('<div class="block-header">🛡️ COMPARADOR DE ALARMAS — PAQUETES BÁSICOS HOGAR</div>', unsafe_allow_html=True)
    st.markdown("""
        <div style="background:#161b22; border:2px solid #d2ff00; border-radius:15px; padding:25px; text-align:center; margin-bottom:25px;">
            <h2 style="color:#d2ff00; margin-bottom:10px;">🔗 Comparador de Alarmas</h2>
            <p style="color:#8b949e; font-size:1rem; margin:0;">Comparativa de los paquetes básicos de Hogar de cada compañía. Usa esta tabla como apoyo durante la llamada con el cliente.</p>
        </div>
    """, unsafe_allow_html=True)

    df_alarmas = pd.DataFrame(tarifas_alarmas).sort_values("PRIORIDAD")
    ...
    for _, row in df_alarmas.iterrows():
        ...
        with col_logo:
            if os.path.exists(row["logo"]):
                ...
                st.markdown(f'<div style="background:#161b22; border:1px solid #30363d; border-radius:10px; padding:20px; text-align:center;"><span style="color:#8b949e; font-size:0.8rem;">{row["COMPAÑÍA"]}</span></div>', unsafe_allow_html=True)
        with col_info:
            st.markdown(f"""
                <div style="background:#161b22; border:1px solid #30363d; border-left:5px solid #d2ff00; border-radius:10px; padding:18px; margin-bottom:14px;">
                    <h3 style="color:white; margin:0 0 8px 0;">{row['COMPAÑÍA']} <span style="color:#d2ff00; font-size:0.9rem;">— {row['PLAN']}</span></h3>
                    <table style="width:100%; color:#c9d1d9; font-size:0.9rem;">
                        <tr><td style="padding:3px 0; width:140px;"><b>💰 Cuota:</b></td><td>{row['CUOTA']}</td></tr>
                        <tr><td style="padding:3px 0;"><b>🔧 Instalación:</b></td><td>{row['INSTALACION']}</td></tr>
                        <tr><td style="padding:3px 0;"><b>📅 Permanencia:</b></td><td>{row['PERMANENCIA']}</td></tr>
                        <tr><td style="padding:3px 0; vertical-align:top;"><b>📦 Elementos:</b></td><td>{row['ELEMENTOS']}</td></tr>
                    </table>
                </div>
            """, unsafe_allow_html=True)

    st.caption("💡 Precios y elementos correspondientes al paquete básico/entrada de gama de cada compañía para vivienda Hogar. Consulta el Repositorio para ver las tarifas completas en PDF.")
